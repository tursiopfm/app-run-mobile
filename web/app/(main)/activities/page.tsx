...
export default async function ActivitiesPage({
  ...
  const [{ data: rows }, { data: profile }, mode] = await Promise.all([
    supabase
      .from('activities')
      .select('id, name, sport_type, start_time, ces, avg_hr, max_hr, distance_m, elevation_gain_m, moving_time_sec, computed_intensity, manual_sport_type, manual_intensity, manual_workout_type, manual_distance_m, manual_moving_time_sec, manual_elevation_gain_m')
      .eq('user_id', user.id)
      .is('deleted_at', null)
