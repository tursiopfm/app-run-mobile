...
export function InstallPrompt() {
  ...
  useEffect(() => {
    ...
    const onBeforeInstall = (e: Event) => {
      ...
      // Slight delay before showing to avoid being intrusive on first paint.
    ...
    // iOS Safari path: no beforeinstallprompt, schedule the manual prompt anyway.
  ...
  if (showIosInstructions) {
    return (
      <div
        role="dialog"
        aria-modal="true"
        aria-label="Installer Trail Cockpit"
        ...
        <div className="w-full max-w-md bg-trail-card border border-trail-border rounded-2xl p-5 shadow-2xl">
          <div className="flex items-start gap-3 mb-4">
            ...
            <button
              ...
              aria-label="Fermer"
              className="text-trail-muted hover:text-trail-text p-1 -m-1"
            >
              <X size={18} />
            </button>
          </div>
    ...
    >
      <div className="bg-trail-card border border-trail-border rounded-2xl p-4 shadow-2xl flex items-center gap-3">
        ...
        <button
          ...
          className="shrink-0 text-trail-muted hover:text-trail-text p-1 -m-1"
        >
          <X size={18} />
        </button>
      </div>
