...
export function InstallPrompt() {
  ...
  if (showIosInstructions) {
    return (
      <div role="dialog" aria-modal="true" aria-label={I.title}
        ...
        <div className="w-full max-w-md bg-trail-card border border-trail-border rounded-2xl p-5 shadow-2xl">
          <div className="flex items-start gap-3 mb-4">
            ...
            <div className="flex-1">
              <h2 className="text-trail-text font-semibold text-base">{I.title}</h2>
              <p className="text-trail-muted text-13 mt-0.5">{I.iosBody}</p>
            </div>
            <button onClick={handleDismiss} aria-label={I.closeAria} className="text-trail-muted hover:text-trail-text p-1 -m-1">
              <X size={18} />
            </button>
          </div>
          <ol className="space-y-3">
            <li className="flex items-center gap-3 bg-trail-surface border border-trail-border rounded-xl p-3">
              ...
              <span className="text-trail-text text-sm flex-1">{I.iosStep1}</span>
            ...
            <li className="flex items-center gap-3 bg-trail-surface border border-trail-border rounded-xl p-3">
              ...
              <span className="text-trail-text text-sm flex-1">
                {I.iosStep2Lead} <span className="font-semibold">{I.iosStep2Choice}</span>
              </span>
            ...
            <li className="flex items-center gap-3 bg-trail-surface border border-trail-border rounded-xl p-3">
              ...
              <span className="text-trail-text text-sm flex-1">{I.iosStep3}</span>
            </li>
          </ol>
  ...
  return (
    <div role="dialog" aria-label={I.title}
      ...
      <div className="bg-trail-card border border-trail-border rounded-2xl p-4 shadow-2xl flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-trail-text font-semibold text-sm leading-tight">{I.title}</p>
          <p className="text-trail-muted text-xs mt-0.5 leading-snug">{I.bannerSub}</p>
        </div>
        ...
        <button onClick={handleDismiss} aria-label={I.laterAria} className="shrink-0 text-trail-muted hover:text-trail-text p-1 -m-1">
          <X size={18} />
        </button>
      </div>
    </div>
