  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/60" onClick={onClose}>
      ...
      >
        ...
        {typeof body === 'string' ? (
          <p className="text-body-sm text-trail-muted leading-[19px] whitespace-pre-line">{body}</p>
        ) : (
          <div className="text-body-sm text-trail-muted leading-[19px]">{body}</div>
        )}
        <button
          onClick={onClose}
          className="mt-5 w-full py-2.5 rounded-[10px] bg-trail-surface border border-trail-border text-body font-semibold text-trail-text"
        >
          Fermer
        </button>
      </div>
