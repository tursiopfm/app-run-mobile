...
- **Constat 2026-07-25** (relevé en enquêtant sur le Cockpit figé) : les crons existent bien — `.github/workflows/strava-import-cron.yml` (toutes les 5 min) et `strava-streams-backfill-cron.yml` (toutes les 10 min), via GitHub Actions car Vercel Hobby plafonne les crons à once-per-day — mais **aucun n'importe les nouvelles activités d'un compte déjà importé** : `app/api/cron/strava-import/route.ts:34` ne sélectionne que `import_status ∈ ('pending','in_progress')` (le compte de Franck est `completed` depuis le 2026-06-09 → chaque tick renvoie `processed: 0`), et le backfill n'écrit que `activity_streams` / `activity_metrics`. Le webhook (`app/api/webhooks/strava/route.ts:62`) empile dans `sync_jobs` sans **aucun consommateur** dans le dépôt. Tous les imports quotidiens viennent donc du client (`SyncOnFocus` / pull-to-refresh) : ça marche tant que l'app est ouverte, ça décroche dès qu'elle ne l'est pas. → le « sync de sécurité quotidien » ci-dessus est le chaînon manquant (ou un consommateur de `sync_jobs`).
