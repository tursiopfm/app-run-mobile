...
- **Constat 2026-07-25** (relevé en enquêtant sur le Cockpit figé) : il n'existe **aucune** ingestion automatique côté serveur. `app/api/cron/strava-import/route.ts` ne traite que les users dont `import_status ∈ ('pending','in_progress')` — un compte `completed` n'est jamais repassé ; `vercel.json` ne déclare d'ailleurs aucun `crons`. Et `app/api/webhooks/strava/route.ts` insère dans `sync_jobs` sans **aucun consommateur** dans le dépôt. Tous les imports quotidiens viennent donc du client (`SyncOnFocus` / pull-to-refresh) : ça marche tant que l'app est ouverte, ça décroche dès qu'elle ne l'est pas.
